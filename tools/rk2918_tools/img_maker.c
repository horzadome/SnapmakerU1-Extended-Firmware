...
int read_metadata(const char *metafile, struct _rkfw_header *rom_header)
{
	FILE *fp = fopen(metafile, "r");
	if (!fp)
	{
		fprintf(stderr, "Can't open metadata file %s: %s\n", metafile, strerror(errno));
		return -1;
	}

	int found_chip = 0, found_time = 0;
	char line[256];
	while (fgets(line, sizeof(line), fp))
	{
		...
		if (sscanf(line, "chip=0x%x", &val_hex) == 1)
		{
			rom_header->chip = val_hex;
			found_chip = 1;
		}
		else if (sscanf(line, "code=0x%x", &val_hex) == 1)
			rom_header->code = val_hex;
		else if (sscanf(line, "build_time=%d-%d-%d %d:%d:%d",
			&year, &month, &day, &hour, &minute, &second) == 6)
		{
			rom_header->year = year;
			rom_header->month = month;
			rom_header->day = day;
			rom_header->hour = hour;
			rom_header->minute = minute;
			rom_header->second = second;
			found_time = 1;
		}
	}
	...
	if (!found_chip || !found_time)
	{
		fprintf(stderr, "Invalid metadata file %s: missing required fields\n", metafile);
		return -1;
	}
...
int pack_rom(const char *loader_filename, const char *image_filename, const char *outfile, const char *metafile)
{
	time_t nowtime;
	...
	if (metafile)
	{
		if (read_metadata(metafile, &rom_header) != 0)
			goto pack_fail;
		fprintf(stderr, "using metadata from: %s\n", metafile);
	}
...
int main(int argc, char **argv)
{
	if (argc == 4 || argc == 5)
	{
		return pack_rom(argv[1], argv[2], argv[3], argc == 5 ? argv[4] : NULL);
	}
	else
	{
		fprintf(stderr, "usage: %s <loader> <old image> <out image> [metadata]\n", argv[0]);
	}
