...
int unpack_update(const char* srcfile, const char* dstdir) {
	...
	if (header.num_parts) {
		...
		for (i = 0; i < header.num_parts; i++) {
			...
			printf("Unpacking: name=%s filename=%s\tnand_addr=%u/%u\tpos=%u/%u/%u\n",
				part->name, part->filename,
				part->nand_addr, part->nand_size,
				part->pos, part->size, part->padded_size);
	...
	if (parts) {
		*parts = '\0';
		...
		for (; part; part = strtok_r(NULL, ",", &token1)) {
			...
			ptr = strchr(ptr, '@');
			if (*part == '-' && ptr == part+1) {
				// size = "-" means use whole remaining space
				p_part->size = (unsigned int)-1;
			}
			if (!ptr)
				continue;
			...
			// since parameter.txt uses `userdata:grow` but we want just `userdata`
			char *drop_grow = strstr(p_part->name, ":grow");
			if (drop_grow) {
				*drop_grow = '\0';
			}
...
int pack_update(const char* srcdir, const char* dstfile) {
	...
	snprintf(buf, sizeof(buf), "%s/%s", srcdir, "parameter.txt");
	...
	for (i = 0; i < package_image.num_package; ++i) {
		struct update_part *pack = &header.parts[i];
		strcpy(pack->name, package_image.packages[i].name);
		strcpy(pack->filename, package_image.packages[i].filename);
		pack->nand_addr = package_image.packages[i].nand_addr;
		pack->nand_size = package_image.packages[i].nand_size;

		if (strcmp(pack->filename, "SELF") == 0)
			continue;

		int j;

		for (j = 0; j < i; j++) {
			struct update_part *matching = &header.parts[j];
			if (strcmp(pack->filename, matching->filename) != 0)
				continue;

			pack->pos = matching->pos;
			pack->size = matching->size;
			pack->padded_size = matching->padded_size;

			printf("Re-using: name=%s from=%s filename=%s nand_addr=%u/%u pos=%u/%u/%u\n",
				pack->name, matching->name, pack->filename,
				pack->nand_addr, pack->nand_size,
				pack->pos, pack->size, pack->padded_size);
			break;
		}

		if (j == i) {
			snprintf(buf, sizeof(buf), "%s/%s", srcdir, pack->filename);
			import_package(fp, pack, buf);

			// for some reason the `padded_size` is aligned to be multiplied by 2048
			pack->padded_size = (pack->size + 2047) / 2048;

			printf("Added: name=%s path=%s nand_addr=%u/%u pos=%u/%u/%u\n", pack->name, buf,
				pack->nand_addr, pack->nand_size, pack->pos, pack->size, pack->padded_size);
		}
	}
